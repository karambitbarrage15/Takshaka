  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4 dark:border-zinc-800">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-400">LLD Architectural Scorecard</span>
            </div>
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-800/50">
              ...
              <ul className="mt-2.5 space-y-1.5">
                {feedback.strengths.length > 0 ? (
                  ...
                  <li className="text-xs text-zinc-400">None highlighted</li>
                )}
              </ul>
            </div>
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-800/50">
              ...
              <ul className="mt-2.5 space-y-1.5">
                {feedback.improvements.length > 0 ? (
                  ...
                  <li className="text-xs text-zinc-400">None required</li>
                )}
              </ul>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-3 space-y-3">
              {feedback.rubricEvaluations.map((item, idx) => (
                <div
                  key={idx}
                  className="rounded-xl border border-zinc-200 bg-white p-4 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/60"
                >
                  ...
                  {/* Evidence */}
                  <div className="mt-2.5 text-xs text-zinc-600 dark:text-zinc-300">
                    <span className="font-semibold text-zinc-800 dark:text-zinc-200">Evidence: </span>
                    {item.evidence}
                  </div>
        ...
        {/* Footer Actions */}
        <div className="flex items-center justify-between border-t border-zinc-200 bg-zinc-50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900/90">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-zinc-300 bg-white px-4 py-2 text-xs font-semibold text-zinc-700 shadow-xs transition hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
          >
            Inspect Canvas
          </button>
          <button
            type="button"
            onClick={onRetry}
            className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-blue-500"
          >
